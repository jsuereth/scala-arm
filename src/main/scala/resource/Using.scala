...
object Using extends UsingCompat {
  ...
  def bufferedOutputStream(out: => OutputStream): ManagedResource[BufferedOutputStream] = managed(out).map(new BufferedOutputStream(_))

  /** Converts a function which creates an InputStream into a ManagedResource[BufferedInputStream]. */
  def bufferedInputStream(in: => InputStream): ManagedResource[BufferedInputStream] = managed(in).map(new BufferedInputStream(_))
  ...
  def fileReader(charset: Charset)(source: File): ManagedResource[BufferedReader] = file(f => new BufferedReader(new InputStreamReader(new FileInputStream(f), charset)))(source)
  ...
  /** 
   * Constructs a new outpustream which ensures the GZIP is "finished" 
   * after completing our operation.
   * 
   * *Note:* Default buffer size is 8192.
   */
